        ...
        for (int i = 1; i <= 3; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                for (int k = 0; k < 4; k++) {
                    ...
                    List htmlList = HTMLWorker.parseToList(new StringReader("<html><p>2017年8月17日，LIGO和Virgo在4000万秒差距（1.3亿光年）之外的NGC\n 4993星系内首次探测到了两颗中子星的合并</p>\n</html>"), styleSheet);
                    for (Object aHtmlList : htmlList) {
                        Element e = (Element) aHtmlList;
                        context.add(e);
                        BaseFont baseFont1 = BaseFont.createFont(SHIPPED_FONT_RESOURCE_PATH, BaseFont.IDENTITY_H, BaseFont.EMBEDDED);//创建一个支持中文的字体
                        Font chinese = new Font(baseFont1, 10, Font.BOLDITALIC, Color.red);//
                    ...
                    table.addCell(createCell(context));
